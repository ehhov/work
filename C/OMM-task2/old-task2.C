...
const uint Nx=257, Ny=257, Nt=513, k0=16;
...
double func(double x, double y, double t) {return sin(pi*x)*t*y*(y-1);}//{return sin(3.0*pi*x)*exp(-t);}
...
int main () {
	...
	if (is_max==1) {
		...
		cerr << "Maximum " << time << " at time " << par_k*tau << endl;
	}
